(() => {
  function initFileTree(options) {
    ...
    const iconBaseUrl = opts.iconBaseUrl || '/assets/icons/minimal/';
    const iconBase = String(iconBaseUrl).endsWith('/')
      ? String(iconBaseUrl)
      : String(iconBaseUrl) + '/';
    ...
    const buildTree = (nodes, parentList) => {
      ...
      nodes.forEach((node) => {
        ...
        if (node.kind === 'folder') {
          ...
          const childHasActive = buildTree(
          ...
          const noteIcon = createIconElement(
            node.icon || 'markdown.svg',
            'filetree-icon filetree-note-icon',
            'document.svg',
          );
          if (noteIcon) {
